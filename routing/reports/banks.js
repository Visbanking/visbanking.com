...
router.get("/", (req, res) => {
	if (!req.query.state && !req.query.city && !req.query.status && !req.query.bankName) {
		connection.query("SELECT * FROM Visbanking.AllReports WHERE BankName IS NOT NULL AND State <> '' AND Type = 'Bank' GROUP BY IDRSSD ORDER BY BankName ASC;", (err, results, fields) => {
			if (err) {
		...
		if (bankName && state && city && status)
			res.redirect(
				`/reports/bank/${toUpper(state)}/${city
					.split(" ")
					.map((word) => capitalize(word))
					.join(" ")}?status=${status}&bankName=${bankName}`
			);
		else if (bankName && state && city)
			res.redirect(
				`/reports/bank/${toUpper(state)}/${city
					.split(" ")
					.map((word) => capitalize(word))
					.join(" ")}?bankName=${bankName}`
			);
		else if (bankName && state && status) res.redirect(`/reports/bank/${toUpper(state)}?status=${status}&bankName=${bankName}`);
		else if (state && city && status)
			res.redirect(
				`/reports/bank/${toUpper(state)}/${city
					.split(" ")
					.map((word) => capitalize(word))
		...
		else if (bankName && state) res.redirect(`/reports/bank/${toUpper(state)}?bankName=${bankName}`);
		else if (state && city)
			res.redirect(
				`/reports/bank/${toUpper(state)}/${city
					.split(" ")
		...
		else if (state && status) res.redirect(`/reports/bank/${toUpper(state)}?status=${status}`);
		else if (state) res.redirect(`/reports/bank/${toUpper(state)}`);
		else res.redirect("/reports/bank");
	}
});
...
router.get("/:state_abbrevitation", (req, res) => {
	...
	connection.query(`SELECT * FROM Visbanking.AllReports WHERE BankName IS NOT NULL AND State = '${toUpper(state)}' AND Type = 'Bank' GROUP BY IDRSSD ORDER BY BankName ASC;`, (err, results, fields) => {
		if (err) {
			...
			if (results.length === 0) {
				return res.redirect("/reports/bank");
			}
			res.render("banks", {
				banks: results,
...
router.get("/:state_abbreviation/:city_name", (req, res) => {
	...
	connection.query(`SELECT * FROM Visbanking.AllReports WHERE BankName IS NOT NULL AND State = '${toUpper(state)}' AND City = '${toUpper(city)}' GROUP BY IDRSSD ORDER BY BankName ASC;`, (err, results, fields) => {
		if (err) {
			...
			if (results.length === 0) {
				return res.redirect(`/reports/bank/${state}`);
			}
			res.render("banks", {
				banks: results,
				title: `Banks in ${city
					.split(" ")
					.map((word) => capitalize(word))
					.join(" ")}, ${toUpper(state)}, US - Visbanking`,
				path: req.originalUrl,
				state,
				city: city
					.split(" ")
		...
		if (err) {
			...
			if (tiers.indexOf(req.cookies.tier) >= tiers.indexOf(tier)) {
				getPDFUrl("ds-allreports", `individual-bank/pdf/call-report/${bank}.pdf`)
					...
					.catch(() => {
						connection.query(
							`SELECT BankName, City, State, IDRSSD, Status FROM Visbanking.AllReports WHERE FileExtension = 'html' AND Tier <> 'Free' AND State = '${state}' AND Status = 'Active' ORDER BY RAND() LIMIT 0, 3;`,
							(err, results, fields) => {
								res.render("reports/bank", {
									path: req.originalUrl,
									access: true,
...
router.get("/:state_abbreviation/:city_name/:bank_id/:report_page_name", (req, res) => {
	...
	connection.query(
		`SELECT BankName, Tier, URL FROM Visbanking.AllReports WHERE State = '${toUpper(state)}' AND City = '${toUpper(city)}' AND IDRSSD = ${toUpper(bank)} AND FileExtension = 'html' AND Tier <> 'Free';`,
		async (err, results, fields) => {
			if (err) {
				res.redirect(`/reports/bank/${state}/${city}/${bank}/${page}`);
			} else {
				...
				if (tiers.indexOf(req.cookies.tier) >= tiers.indexOf(tier)) {
					const fileToRequest = (page_name) => {
						...
						else return res.redirect(`/reports/bank/${state}/${city}/${bank}/bank`);
					};
					...
					if (req.cookies.tier === "Free" || !req.cookies.tier) {
						if (page !== "general") return res.redirect(`/reports/bank/${state}/${city}/${bank}/general`);
						connection.query(
							`SELECT BankName, URL from Visbanking.AllReports WHERE FileExtension = 'html' AND Tier = 'Free' AND State = '${toUpper(state)}' AND City = '${toUpper(city)}' AND IDRSSD = ${toUpper(bank)};`,
							(err, results, fields) => {
								if (err) {
									res.redirect(`/reports/bank/${state}/${city}/${bank}/general`);
								} else {
									readFile("ds-allreports-free", `call-report/${bank}/index.html`)
										...
										.catch(() => {
											connection.query(
												`SELECT BankName, City, State, IDRSSD, Status FROM Visbanking.IndividualBankHTMLReports WHERE BankName IS NOT NULL State = '${state}' AND Status = 'Active' ORDER BY RAND() LIMIT 0, 3;`,
												(err, results, fields) => {
													res.render("reports/bank", {
														path: req.originalUrl,
														access: true,
