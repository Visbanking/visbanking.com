...
router.get("/connect/linkedin", (req, res) => {
	if (!req.query.state)
		res.redirect(
			`https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=${process.env.LINKEDIN_CLIENT_ID}&redirect_uri=https://visbanking.com/me/connect/linkedin&state=${process.env.LINKEDIN_STATE_STRING}&scope=r_liteprofile%20r_emailaddress`
		);
		...
		if (req.query.error) {
			...
			post(
				`https://www.linkedin.com/oauth/v2/accessToken?grant_type=authorization_code&code=${req.query.code}&client_id=${process.env.LINKEDIN_CLIENT_ID}&client_secret=${process.env.LINKEDIN_CLIENT_SECRET}&redirect_uri=https://visbanking.com/me/connect/linkedin`,
				{},
				{
					headers: {
